...
export const MarketSummaryTable: React.FC<Props> = ({
  headers,
  headerStyles,
  rows,
  rowStyles
    ...
    <tbody className="flex flex-col divide-y divide-hl-border px-4">
      {rows.map((row, i) => (
        <tr
          ...
          key={`table-rows-${i}`}
        >
          <td className={classNames("col-span-2")} key={`table-row-data-0`}>
            {row[0]}
          </td>
          <td className={classNames("col-span-2")} key={`table-row-data-1`}>
            {row[1]}
          </td>
          <td className={classNames("col-span-4")} key={`table-row-data-2`}>
            {row[2]}
          </td>
          <td className={classNames("col-span-4")} key={`table-row-data-3`}>
            {row[3]}
          </td>
        </tr>
      ))}
